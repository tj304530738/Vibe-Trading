...
export function Settings() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      toast.error(`保存大模型设置失败：${error instanceof Error ? error.message : "未知错误"}`);
    } finally {
      setSaving(false);
    }
  };
  ...
  const localApiAccessSection = (
    <form onSubmit={submitLocalApiKey} className="rounded-lg border bg-card p-5 shadow-sm">
      <div className="mb-4 space-y-1">
        ...
        <p className="text-sm text-muted-foreground">{"远程或私有部署时，在此浏览器中输入服务器 API 密钥。本地使用可留空。"}</p>
      </div>
      ...
      <p className="mt-2 text-xs text-muted-foreground">{"仅存储在此浏览器中。留空即可清除。"}</p>
    </form>
  );
    ...
    return (
      <div className="mx-auto max-w-5xl space-y-6 p-6">
        <div className="space-y-2">
          ...
          <p className="max-w-3xl text-sm text-muted-foreground">{"为此本地项目配置模型凭证和市场数据源令牌。"}</p>
        </div>
  ...
  const keyStatus = settings.api_key_configured
    ...
    : settings.api_key_required
      ...
      : selectedProvider?.auth_type === "oauth" && selectedProvider.login_command
        ? `此提供商使用 OAuth。运行：${selectedProvider.login_command}`
        : "此提供商不需要 API 密钥。";
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      ...
      <div className="space-y-2">
        ...
        <p className="max-w-3xl text-sm text-muted-foreground">{"选择智能体使用的模型并保存到项目本地的 agent/.env 文件。"}</p>
      </div>
      ...
      <form onSubmit={submit} className="grid gap-6 lg:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.8fr)]">
            ...
            <label className="grid gap-2">
              ...
              <div className="flex gap-2">
                ...
                >
                  ...
                  <span className="hidden sm:inline">{"使用提供商默认值"}</span>
                </button>
              </div>
              <span className={hintClass}>{"请使用提供商要求的确切模型 ID。"}</span>
            </label>
        ...
        <section className="rounded-lg border bg-card p-5 shadow-sm">
          <div className="mb-5 flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold">{"生成"}</h2>
          </div>
          ...
          <div className="grid gap-4">
            ...
            <label className="grid gap-2">
              ...
              <span className={hintClass}>{"模型回答前的思考深度。越高越深入但越慢；选择「关闭」可获得最快回复。"}</span>
            </label>
      ...
      <form onSubmit={submitDataSources} className="rounded-lg border bg-card p-5 shadow-sm">
        <div className="mb-5 space-y-1">
          ...
          <p className="text-sm text-muted-foreground">{"配置回测和研究智能体使用的可选市场数据凭证。"}</p>
        </div>
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1.1fr)_minmax(280px,0.9fr)]">
          <div className="grid gap-4">
            <label className="grid gap-2">
              ...
              <div className="flex items-center justify-between gap-3">
                <span className={hintClass}>{"用于中国A股、期货、基金和宏观数据。未设置时项目将回退到 AKShare。"}</span>
                <label className="flex shrink-0 items-center gap-2 text-xs text-muted-foreground">
                  <input
                    type="checkbox"
            ...
            >
              ...
              {dataSaving ? i18n.t("settings.saving") : "保存数据源设置"}
            </button>
          </div>
