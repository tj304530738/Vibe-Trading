...
function getGoalProgress(snapshot: GoalSnapshot | null): {
  ...
  return {
    ...
    metLabel: total > 0 ? `${met}/${total} 已达成` : "",
    evidenceTotal,
  };
}
...
export function Agent() {
  ...
  const handleExport = () => {
    ...
    const lines: string[] = [`# 对话导出`, ``, `导出时间: ${new Date().toLocaleString()}`, ``];
    for (const msg of messages) {
      ...
      if (msg.type === "user") {
        ...
        lines.push(`> 群组状态: ${msg.swarmStatus?.preset || "swarm"} ${msg.swarmStatus?.status || ""}`, ``);
                ...
                <div className="absolute bottom-full left-0 mb-2 w-52 rounded-xl border bg-background/95 backdrop-blur-sm shadow-lg py-1 z-50">
                  ...
                  <button
                    ...
                    onClick={() => {
                      setShowUploadMenu(false);
                      setGoalComposerActive(false);
                      setSwarmPreset({ name: "auto", title: "智能体群组" });
                      inputRef.current?.focus();
                    }}
                    className="w-full px-3 py-2 text-left text-sm hover:bg-muted transition-colors flex items-center gap-2"
                  >
